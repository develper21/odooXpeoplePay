...
export default function EmployeeAttendancePage() {
  ...
  const { data: records = [], isLoading, isError } = useAttendance(id);
  ...
  if (
    ...
    return <ErrorState message="Attendance records could not be loaded." />;
  const canCreate = Boolean(user && canAccess(user.role, "attendance.create"));
  return (
    <>
      <PageHeader
        title={`${employeeName(employee)} · Attendance`}
        description="Actual presence records for this employee."
        action={
          canCreate
            ? {
                label: "Add Attendance",
                href: `/attendance/new?employeeId=${id}`,
              }
            : undefined
        }
      />
      {records.length === 0 ? (
        <EmptyState
          title={`No attendance records for ${employeeName(employee)}`}
          message="Add an attendance record when presence is available."
        />
      ) : (
        <Card>
          <DataTable>
