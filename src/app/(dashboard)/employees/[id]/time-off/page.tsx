...
import { useTimeOff, useTimeOffAllocations, useEmployee } from "@/hooks/use-data";
...
export default function EmployeeTimeOffPage() {
  ...
  const { data: requests = [], isLoading: reqLoading, isError: reqError } = useTimeOff(id);
  const { data: allocations = [], isLoading: allocLoading } = useTimeOffAllocations(id);

  if (empLoading || reqLoading || allocLoading) return <LoadingState />;
  if (!employee || reqError || (user?.role === "EMPLOYEE" && user.employeeId !== id)) {
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          canCreate
            ? { label: "New Request", href: `/time-off/requests/new?employeeId=${id}` }
            : undefined
        }
      />
      ...
      <Card>
        ...
        <CardContent>
          {requests.length === 0 ? (
            <EmptyState title="No time off requests" message="This employee has not submitted any leave requests." />
          ) : (
            <DataTable>
              <TableHeader>
                <TableRow>
              ...
              <tbody>
                {requests.map((req) => (
                  <TableRow key={req.id}>
                    <TableCell className="font-semibold text-text-primary">{req.type}</TableCell>
