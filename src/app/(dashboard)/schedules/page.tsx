  ...
  return (
    <>
      <PageHeader
        ...
        description="Define expected working patterns for your workforce."
        action={
          canCreate
            ? { label: "New Schedule", href: "/schedules/new" }
            : undefined
        }
      />
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          message="Try a different search or create a schedule."
        />
      ) : (
        <DataTable>
          <TableHeader>
            ...
            {filtered.map((schedule) => (
              <TableRow key={schedule.id}>
                ...
                <TableCell className="text-xs text-text-secondary">
                  {(schedule.type || "Fixed").replace(/_/g, " ")}
                </TableCell>
                ...
                <TableCell>
                  {
                    employees.filter(
                      (employee) => employee.scheduleId === schedule.id,
                    ).length
                  }
                </TableCell>
                <TableCell>
