...
// Closes the open attendance for the authenticated user's linked employee.
...
import { attendances, employees } from '@/server/schema';
...
export async function POST(_request) {
  ...
  try {
    ...
    // The open-row predicate makes a concurrent second checkout a no-op.
    const [attendance] = await db
      .update(attendances)
      .set({ clockOut: sql`CURRENT_TIMESTAMP` })
      .where(
        and(
          eq(attendances.employeeId, employee.id),
          isNotNull(attendances.clockIn),
    ...
    if (!attendance) {
      return NextResponse.json({ error: 'No open attendance record found for this employee.' }, { status: 404 });
    }
    ...
    console.error('POST /api/attendance/check-out failed:', err);
