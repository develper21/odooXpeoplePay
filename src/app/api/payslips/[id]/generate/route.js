...
import { calculateEmployeeSalary, SalaryCalculationError } from '@/server/salary-calculator';
import { db } from '@/server/db';
import { contracts, employees, payslipLines, payslips, payruns } from '@/server/schema';
...
export async function POST(request, { params }) {
  ...
  const payrunId = Number(resolvedParams?.id);
  ...
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: 'Invalid payslip generation payload.',
  ...
  try {
    ...
    if (companyId === null) {
      return NextResponse.json({ error: `Payrun ${payrunId} not found.` }, { status: 404 });
    }
    ...
    if (payrun.status !== 'processing') {
      return NextResponse.json(
        { error: 'Payslips can only be generated from a calculated payrun before finalization.' },
        { status: 409 },
      );
    }
    ...
    const [existing] = await db
      ...
      .from(payslips)
      .where(and(eq(payslips.payrunId, payrunId), eq(payslips.employeeId, parsed.data.employee_id)))
      .limit(1);

    if (existing) {
      return NextResponse.json(
        { error: `Payslip already exists for employee ${parsed.data.employee_id} in payrun ${payrunId}.` },
        { status: 409 },
      );
    }
    ...
    const [contract] = await db
      ...
      .where(
        and(
          eq(contracts.employeeId, parsed.data.employee_id),
    ...
    if (
      !contract ||
      contract.startDate > payrun.payPeriodEnd ||
      (contract.endDate && contract.endDate < payrun.payPeriodStart)
    ) {
      return NextResponse.json(
        { error: 'Employee has no eligible active contract for this payrun period.' },
        { status: 422 },
      );
    }

    const result = await calculateEmployeeSalary(parsed.data.employee_id, {
      companyId,
    ...
    if (pgCode === '23505') {
      return NextResponse.json(
        { error: 'Payslip already exists for this employee and payrun.' },
        { status: 409 },
      );
    }
    if (err instanceof SalaryCalculationError) {
