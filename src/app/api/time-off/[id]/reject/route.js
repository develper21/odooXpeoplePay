...
// Rejects a pending, company-scoped time-off request.
...
import { employees, timeOffRequests } from '@/server/schema';
...
export async function POST(_request, { params }) {
  ...
  try {
    ...
    if (pgCode === '23503') {
      return NextResponse.json({ error: 'Rejection references a record that does not exist.' }, { status: 409 });
    }
    console.error('POST /api/time-off/:id/reject failed:', err);
