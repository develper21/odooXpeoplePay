// Single salary component API backed by salary_rules.
...
import { salaryRules, salaryStructures } from '@/server/schema';
...
async function findComponent(id, companyId) {
  const [component] = await db.select(componentColumns).from(salaryRules).innerJoin(salaryStructures, eq(salaryRules.salaryStructureId, salaryStructures.id)).where(and(eq(salaryRules.id, id), eq(salaryStructures.companyId, companyId))).limit(1);
  return component;
}
...
function validateConfiguration(values) {
  if (values.calculationType === 'fixed' && values.amount == null) return 'amount is required for fixed components.';
...
export async function GET(_request, { params }) {
  ...
  if (id === null) return NextResponse.json({ error: 'Invalid salary component id.' }, { status: 400 });
  try {
    const companyId = await getCompanyId();
    const component = companyId == null ? null : await findComponent(id, companyId);
    if (!component) return NextResponse.json({ error: `Salary component ${id} not found.` }, { status: 404 });
    return NextResponse.json({ salary_component: component });
  } catch (err) {
    console.error('GET /api/salary-components/:id failed:', err);
...
export async function PATCH(request, { params }) {
  ...
  if (!parsed.success) return NextResponse.json({ error: 'Invalid salary component payload.', issues: parsed.error.issues.map((issue) => ({ field: issue.path.join('.'), message: issue.message })) }, { status: 400 });
  try {
    const companyId = await getCompanyId();
    if (companyId == null) return NextResponse.json({ error: `Salary component ${id} not found.` }, { status: 404 });
    const existing = await findComponent(id, companyId);
    if (!existing) return NextResponse.json({ error: `Salary component ${id} not found.` }, { status: 404 });
    ...
    const configurationError = validateConfiguration(values);
    if (configurationError) return NextResponse.json({ error: configurationError }, { status: 422 });
    const [component] = await db.update(salaryRules).set(values).where(eq(salaryRules.id, id)).returning(componentColumns);
    if (!component) return NextResponse.json({ error: `Salary component ${id} not found.` }, { status: 404 });
    return NextResponse.json({ salary_component: component });
  } catch (err) {
    const pgCode = err?.code ?? err?.cause?.code;
    if (pgCode === '23505' || /duplicate key/i.test(err?.message ?? '')) return NextResponse.json({ error: 'Component code already exists for this salary structure.' }, { status: 409 });
    if (pgCode === '23503') return NextResponse.json({ error: 'Salary component references a record that does not exist.' }, { status: 409 });
    console.error('PATCH /api/salary-components/:id failed:', err);
    return NextResponse.json({ error: 'Internal server error.' }, { status: 500 });
  }
}
...
export async function DELETE(_request, { params }) {
  ...
  try {
    ...
    const pgCode = err?.code ?? err?.cause?.code;
    if (pgCode === '23503' || /foreign key constraint|violates foreign key/i.test(err?.message ?? '')) return NextResponse.json({ error: 'Cannot delete a salary component referenced by payslip records.' }, { status: 409 });
    console.error('DELETE /api/salary-components/:id failed:', err);
    return NextResponse.json({ error: 'Internal server error.' }, { status: 500 });
  }
}
