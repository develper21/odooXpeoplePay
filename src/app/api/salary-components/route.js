// Salary component API backed by the existing salary_rules table.
...
const componentSchema = z.object({
  ...
  if (component.calculation_type === 'fixed' && component.amount == null) {
    context.addIssue({ code: 'custom', path: ['amount'], message: 'amount is required for fixed components.' });
  }
  if (component.calculation_type === 'percentage' && (component.percentage == null || component.percentage_base == null)) {
...
export async function GET(request) {
  ...
  try {
    ...
    if (companyId === null) return NextResponse.json({ salary_components: [] });
    const structureIdValue = new URL(request.url).searchParams.get('salary_structure_id');
    const filters = [eq(salaryStructures.companyId, companyId)];
    if (structureIdValue !== null) {
      const structureId = Number(structureIdValue);
    ...
    const rows = await db.select(componentColumns).from(salaryRules).innerJoin(salaryStructures, eq(salaryRules.salaryStructureId, salaryStructures.id)).where(and(...filters)).orderBy(asc(salaryRules.salaryStructureId), asc(salaryRules.computationOrder), asc(salaryRules.id));
    return NextResponse.json({ salary_components: rows });
  } catch (err) {
    console.error('GET /api/salary-components failed:', err);
...
export async function POST(request) {
  ...
  const companyId = await getCompanyId();
  if (companyId === null) return NextResponse.json({ error: 'Company profile must be set up before creating salary components.' }, { status: 409 });
  const structureError = await validateStructure(parsed.data.salary_structure_id, companyId);
  if (structureError) return structureError;
  try {
    const [component] = await db.insert(salaryRules).values(ruleValues(parsed.data, parsed.data.salary_structure_id)).returning(componentColumns);
    return NextResponse.json({ salary_component: component }, { status: 201 });
  } catch (err) {
    ...
    console.error('POST /api/salary-components failed:', err);
    return NextResponse.json({ error: 'Internal server error.' }, { status: 500 });
  }
}
