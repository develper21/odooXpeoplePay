...
export function RequestForm({
  ...
  const selectedType = types.find(
    (t) => t.id === values.typeId || t.name === values.typeId,
  );
  ...
  const hasNoAllocation = Boolean(
    selectedType?.allocationRequired && !matchedAllocation,
  );
  const hasInsufficientBalance = Boolean(
    selectedType?.allocationRequired &&
    (hasNoAllocation ||
      (matchedAllocation &&
        usableAllocationRemaining(matchedAllocation) < duration)),
  );
  ...
  const submit = async (raw: FormValues) => {
    ...
    if (hasNoAllocation) {
      setError("endDate", {
        message:
          "No active leave allocation found for this employee. Allocation is required for this leave type.",
      });
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="space-y-6">
      ...
      <div className="rounded-md border bg-surface-raised p-4">
          ...
          <div className="text-right">
            ...
            {matchedAllocation ? (
              <p className="mt-1 text-sm font-semibold text-text-secondary">
                {usableAllocationRemaining(matchedAllocation)}{" "}
                {selectedType?.unit ?? "DAYS"} remaining (out of{" "}
                {matchedAllocation.allocatedDays})
              </p>
            ) : selectedType?.allocationRequired ? (
              ...
              <p className="mt-1 text-xs text-text-muted">
                No allocation required for this type
              </p>
            )}
          </div>
        </div>
        ...
        {hasNoAllocation && (
          <p className="mt-3 text-xs font-semibold text-danger">
            ⚠️ No active leave allocation found for this employee. An allocation
            must be granted before requesting this leave type.
          </p>
        )}

        {!hasNoAllocation && hasInsufficientBalance && (
          <p className="mt-3 text-xs font-semibold text-danger">
            ⚠️ Requested duration ({duration} days) exceeds available balance (
            {matchedAllocation ? usableAllocationRemaining(matchedAllocation) : 0} days).
          </p>
        )}
      </div>
      ...
      <div className="flex justify-end gap-3 border-t pt-5">
        ...
        <Button type="submit" disabled={submitting || hasInsufficientBalance || hasNoAllocation}>
