    ...
    <Card className="group p-5 transition-colors hover:border-border-subtle hover:bg-surface-raised/40">
      <div className="flex items-start justify-between">
        ...
        <span className="flex items-center gap-1 rounded-md bg-surface-raised p-1.5 text-primary">
          ...
          {metric.href && (
            <ChevronRight className="size-3 text-text-muted opacity-0 transition-opacity group-hover:opacity-100" />
          )}
        </span>
      </div>
      <p className="mt-4 text-2xl font-bold tracking-tight truncate" title={metric.value}>
        {metric.value}
      </p>
      <div
        className={
          metric.trend === "up"
            ? "mt-2 flex items-center gap-1 text-xs text-success"
            : "mt-2 flex items-center gap-1 text-xs text-warning"
        }
      >
