...
const schema = z.object({
  employeeNumber: z.string().min(3, "Employee code is required"),
  ...
  scheduleId: z.string().min(1, "Working schedule is required"),
  employeeType: z.enum(["FULL_TIME", "PART_TIME", "CONTRACT"]),
  status: z.enum(["ACTIVE", "INACTIVE", "ON_LEAVE"]),
  joinedOn: z.string().min(1, "Join date is required"),
});
...
export function EmployeeForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      scheduleId: initialValues?.scheduleId ?? schedules[0]?.id ?? "",
      employeeType: initialValues?.employeeType ?? "FULL_TIME",
      status: initialValues?.status ?? "ACTIVE",
      joinedOn:
        initialValues?.joinedOn ?? new Date().toISOString().slice(0, 10),
    },
  });
    ...
    <form onSubmit={handleSubmit(submit)} className="space-y-6">
      <section>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          {field("employeeNumber", "Employee code *")}
